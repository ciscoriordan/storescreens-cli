...
struct ProjectDetector {
    ...
    func detectProjectFile() -> (project: String?, workspace: String?) {
    ...
    func detectScheme(project: String?, workspace: String?) async -> String? {
        var args = ["-list", "-json"]
        if let ws = workspace {
            args += ["-workspace", ws]
        ...
        return parseFirstScheme(from: result.stdout)
    }

    /// Query xcodebuild for the project's IPHONEOS_DEPLOYMENT_TARGET.
    func detectDeploymentTarget(
    ...
    func listTargets(project: String?, workspace: String?) async -> [String] {
        var args = ["-list", "-json"]
        if let ws = workspace {
            args += ["-workspace", ws]
        } else if let proj = project {
    ...
    func findUITestTargets(project: String?, workspace: String?) async -> [String] {
    ...
    func listInstalledRuntimes() async -> [InstalledRuntime] {
