...
struct RunHistoryManager {
    let outputDir: String
    let keepRuns: Int
    let logger: Logger

    /// Prepare the directory where the current capture should write screenshots.
    func prepareCaptureDirectory() throws -> CaptureDestination {
    ...
    func finalizeCapture(_ destination: CaptureDestination) throws {
        let fm = FileManager.default
    ...
    func handleFailure(_ destination: CaptureDestination) {
    ...
    ///   (e.g. a previous run's `iPad_Pro_13/` when the new run only has `iPhone_6.9/`)
    ///   untouched.
    /// - Plain files (e.g. `manifest.json`) are overwritten.
    private func mergeDirectory(from src: String, into dst: String) throws {
        ...
        // no subdirectories. Directories that contain both .png files and subdirs (e.g. the
        // staging root with AppIcon.png + locale dirs) must NOT be treated as leaves.
